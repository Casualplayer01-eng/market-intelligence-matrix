...
- **GDP growth (40%)** calculated from live World Bank data using a simple historical average.
...
Economic projections (2035–2050) are generated using Gaussian stochastic shocks (σ = 0.005), providing forward-looking trends while accounting for macroeconomic volatility.
 
---

## Architecture

- **ETL pipeline:** Python-based automation running via GitHub Actions to fetch live World Bank data and persist the results to `market_engine_cache.csv`. Includes a resilient connection pool with exponential backoff retries (handling HTTP 429/50x) and automatic fallback to previous cached data if API limits occur.
...
- **Result:** Markets are grouped into "Target," "Watch," or "Avoid" categories using quantile distribution (`pd.qcut`), providing clear actionable business insights.
